...
- Список і деталі статей, створення / редагування для авторів.
- Автори, профілі, закладки (збережені статті).
- Автентифікація через httpOnly cookies; завантаження аватара (Cloudinary).
- Next.js BFF проксує `/api/*` на Express і передає cookies.
- Власний live: Vercel (UI) + Render (API).
...
| Runtime | Node.js 22.18.0 (див. `frontend/.nvmrc`) |
